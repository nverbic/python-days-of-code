''' 
    ...
    [1904, 1908, 1912, 1916, 1920, 1924, 1928, 1932, 1936, 1940, 1944, 1948, 1952, 1956, 1960,
     ...
     2580, 2584, 2588, 2592, 2596, 2604, 2608, 2612, 2616, 2620, 2624, 2628, 2632, 2636, 2640,
     2644, 2648, 2652, 2656, 2660, 2664, 2668, 2672, 2676, 2680, 2684, 2688, 2692, 2696, 2704,
     ...
     2888, 2892, 2896, 2904, 2908, 2912, 2916, 2920, 2924, 2928, 2932, 2936, 2940, 2944, 2948,
     2952, 2956, 2960, 2964, 2968, 2972, 2976, 2980, 2984, 2988, 2992, 2996]

    Leap years do not always occur every 4 years! The exceptions are:
    [1900, 2100, 2200, 2300, 2500, 2600, 2700, 2900, 3000]
'''

def is_leap_year(years):
    '''' Find the leap years and the exceptions to the rule 
        (every 4th year in not always the leap year)
    '''
    leap_years = []
    exception_years = []
    ...
    for item in years:
        if not item % 4:
            if item % 100:
                ...
                exception_years.append(item)

    print(f"\nLeap years from {years[0]} to {years[-1]}:\n{leap_years}")
    print(f"\nLeap years do not always occur every 4 years! The exceptions are:\n{exception_years}\n")



if __name__ == "__main__":
    # List of years from 1900 to 3000
    years = list(range(1900, 3001))

    # Find the leap years in the list
    is_leap_year(years)
